...
| `UPGRADE-*.md` inside the read and write boundary | A deprecation already declares its target branch's upgrade file in the change record's own `target.paths`, which is what lets C2 and C4 pass. That declaration is per record and never reaches `boundary.read_allow`, which is scoped to the component and does not vary per change. A real `/deprecate` run wrote `UPGRADE-8.2.md` and was then denied reading back what it had just written. Widened for every branch's file rather than one, since the boundary is static and the next deprecation will not target 8.2. Same maintenance loop as `.agents/**`: denied, asked, widened by the user in the manifest |
